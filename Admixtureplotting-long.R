...
axis(2, line=-5,seq(0,5000,1000),font=1)
title(ylab="Elevation (m)", line=-1, cex.lab = 1.5)
segments(x0=0,y0=1000,x1=897,y1=1000,col="Black",lwd = 0.5, lty = "dotted")
segments(x0=0,y0=2000,x1=897,y1=2000,col="Black",lwd = 0.5, lty = "dotted")
segments(x0=0,y0=3000,x1=897,y1=3000,col="Black",lwd = 0.5, lty = "dotted")

#segments(x0=0,y0=0,x1=0,y1=4000,col="Black")
segments(x0=0,y0=0,x1=897,y1=0,col="Black")
...
axis(1,line=-0.1,at = breaks[1:(length(breaks)-1)]+((0:(length(breaks)-2))*2)+((breaks[2:length(breaks)]-breaks[1:(length(breaks)-1)])/2),lwd=0,lwd.ticks=1,labels = gsub("_"," ",tbl$Source[breaks]), las=2)
axis(2, line=-5,seq(0.25,0.75,0.25),font=1)
...
#cv <- c(0.76528,0.72507,0.70422,0.68975,0.68528,0.68207,0.67936,0.68012,0.68167,0.68476,0.68918,0.68971,0.69282,0.6979,0.70267)
...
abline(v=6, col="red", lty=2, lwd= 1.5)
...
par(mar=c(.1,.1,.1,.1))
plot(1:(k+3),1:(k+3),col=col[1:k],pch=15,cex=.0001,frame.plot=F,main="Modelled FST K=6",line=-2.3,xaxt="n",yaxt="n",xpd=T,ylab="",xlab="")
