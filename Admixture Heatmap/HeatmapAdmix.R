...
heat1 <- pheatmap (tblscaled,
                   cluster_rows = TRUE, show_colnames = T,show_rownames = F, fontsize_col=15, cluster_cols = FALSE,cutree_rows = 10)
dev.off()

clust_names <- cutree(heat1$tree_row, k=10) ### cut into 5clusters..decided based on your data visulaization
Clustered_data <- data.frame (tblscaled, clust_names)
head (Clustered_data)
write.table(Clustered_data, file="All_landrace_pheatmap_clusters.txt",
            sep="\t", quote=F)
